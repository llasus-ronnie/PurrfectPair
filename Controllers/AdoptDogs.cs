...
{
    public class AdoptDogs : Controller
    {

        private readonly ApplicationDbContext _context;
        private readonly ILogger<ViewStatusController> _logger;

        public AdoptDogs(ILogger<ViewStatusController> logger, ApplicationDbContext context)
        {
            _logger = logger;
            _context = context;
        }
        public IActionResult Index()
        {
            var petSubmissionModel =
              _context.PetSubmissions.Where(x => x.pet_type == "Dog" && !x.pet_is_adopting)
